...
namespace Interpolation
{
    public partial class Form1 : Form
    {
        ...
        private void btnSolveSpline_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SolveQuadraticSpline(x, y, precision, out double[] a, out double[] b, out double[] c, out double[] h, out double[] m, out double[] v);
                    DisplayQuadraticSplineResults(rtbResultSpline, x, y, a, b, c, h, m, v, precision);
                }
    ...
    out double[] m, out double[] v)
        {
            ...
            for (int i = 0; i < n - 1; i++)
            {
                ...
                if (h[i] == 0)
                {
                    throw new ArgumentException($"Các mốc x bị trùng nhau tại chỉ số {i} và {i + 1}.");
                }

                v[i] = Math.Round(2 * (y[i + 1] - y[i]) / h[i], precision);
            }

            m[0] = 0; // Điều kiện biên
    ...
    double[] coeffsVector, double[,] productTable, double[] gaussIPolynomial)
        ...
        private void DisplayQuadraticSplineResults(RichTextBox rtb, double[] x, double[] y, double[] a, double[] b, double[] c, double[] h, double[] m, double[] v, int precision)
        {
            ...
            sb.AppendLine("───────────────────────────────────────────────────────────────");
            sb.AppendLine($"  S'(x₀) = m[0] = 0");
            sb.AppendLine("  (Đạo hàm bậc nhất tại điểm đầu bằng 0)\n");
